let itemsDB = [], hideoutDB = [], overridesDB = {};
    ...
    dom.loaderText.textContent = `LOADING LANGUAGE: ${String(lang || '').toUpperCase()}...`;

    try {
        const response = await fetch(`${GITHUB_RAW_BASE_URL}languages/${lang}.json`);
    ...
    try {
        const bundleUrl = `${GITHUB_RAW_BASE_URL}db/arc.bundle.json`;

        dom.loaderText.textContent = 'DOWNLOADING DATABASE BUNDLE...';
        ...
        // Expected shape
        // bundle.items: array
        // bundle.hideout: array
        itemsDB = Array.isArray(bundle.items) ? bundle.items : [];
        const rawHideout = Array.isArray(bundle.hideout) ? bundle.hideout : [];
        hideoutDB = normalizeHideoutData(rawHideout, 'RaidTheory');

        window.__dbLoaded = true;
        ...
        console.error(error);
        dom.loader.style.display = 'none';
        alert('Connection Failed. Bundle missing or blocked. Check console.');
    }
}
...
(function(){
  function resetDatabase(){
    try{ itemsDB=[]; hideoutDB=[]; overridesDB={}; wishlist.clear(); ownedBlueprints.clear(); window.__dbLoaded=false; }catch(_){ }
